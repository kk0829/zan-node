...
function controller(...args) {
    ...
    return target => {
        ...
        proto.$routes = Object.getOwnPropertyNames(proto).filter(prop => prop.indexOf(PREFIX) === 0).map(prop => proto[prop]).reduce((prev, next) => prev.concat(next), []).map(prop => {
